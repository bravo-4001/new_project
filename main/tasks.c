...
void can_seq_trans_task(void *params)
{
    ...
    while (1)
    {
        ...
        uint32_t buffer = (uint32_t)(transmit_data.TargetVoltage * 10);
        ...
        buffer = (uint32_t)(transmit_data.TargetCurrent * 10);
        ...
        // { // second seq
        payload2[0] = 2;
        ESP_LOGI("CAN send task", "MaximumPower to be sent: %.2f", transmit_data.MaximumPower);
        buffer = (uint32_t)(transmit_data.MaximumPower * 10);

        payload2[1] = buffer & (0xff);
        buffer >>= 8;
        payload2[2] = buffer & (0xff);
        ...
        err = transmit_seq_1(payload2);
        printf("Transmit result = %s (%d)\n", esp_err_to_name(err), err);
        vTaskDelay(pdMS_TO_TICKS(50)); // Add a small delay to allow other tasks to run
    }
}
